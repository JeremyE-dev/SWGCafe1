...
namespace SWG_Warmups.BLL
{
    public class Warmups
    {
        ...
        {
            string s = specialLetter;
            double newTotal = orderTotal/2;
        ...
        public double RandomLetterDiscount(string Name, char specialLetter, int lettertoCheck, double orderTotal)
        {
            ...
            string s = specialLetter.ToString();
          
            string lastLetterInString = Name[Name.Length - 1].ToString();


            //if out of range check last letter
            if ( lettertoCheck >= Name.Length-1)
            {
                if(s.Equals(lastLetterInString.ToUpper()) || s.Equals(lastLetterInString.ToLower()))
                {
                    return newTotal;
                }
            ...
            string letterAtIndex = Name[lettertoCheck].ToString();

            //if in range check letter at index
            if (lettertoCheck <= Name.Length - 1)
            {
                if (s.Equals(letterAtIndex.ToUpper()) || s.Equals(letterAtIndex.ToLower()))
                {
                    return newTotal;
                }
        ...
        public string CheckingForTypos(string firstName)
        {
            ...
            if (firstName[firstName.Length - 1].Equals(firstName[firstName.Length-2]))
            {
                newString = newString.Substring(0, firstName.Length - 1);

            }

            if (firstName[0].Equals(firstName[1]))
            {
        ...
        public string QualityAssurance (string firstName)
        {
            ...
            if(!firstName.EndsWith("l") && !firstName.EndsWith("L") && !firstName.EndsWith("e") && !firstName.EndsWith("E"))
            {
        ...
        public string FunnyNameDay(string firstName)
        {
            ...
            {
                newString += firstName.Substring(0, i);
            }

            newString = newString + firstName;
        ...
        public int CustomerSurvey(int[] surveyResults)
        {
            ...
            foreach(int x in surveyResults) 
            {
                if (x >= 8)
                {
                    count++;
        ...
        public bool AreCustomersHappy(int[] surveyResults)
        {
            ...
            average = sum / surveyResults.Length;
        ...
        public double WeekendAverage(int[] saturdayResults, int[] sundayResults)
        {
            ...
            int sunSum = 0;
            int totalSum = 0;
            int combinedLength = saturdayResults.Length + sundayResults.Length;
            double average = 0;

            foreach(int x in saturdayResults)
            {
                satSum += x;
            }

            foreach (int x in sundayResults)
            {
                sunSum += x;
            }

            totalSum = satSum + sunSum;

            average = totalSum / combinedLength;
        ...
        {
            int normalStaffing = 6;
            int staffNeeded = 0;

            if (isWinter && isWeekend)
            {
                return normalStaffing + 5;
            }

            if(isWinter)
            {
                return normalStaffing + 2;
            }

            else if (isWeekend)
            {
                return normalStaffing + 3;
            }

            else
            {
                return normalStaffing;
            }
        ...
        public bool PhoneSupport(bool isStoreOpen, bool isMorning, bool isDistrictManager)
        {
            ...
            if (isStoreOpen && isMorning && !isDistrictManager)
            {
                answerPhone =  false;
            }

            if (!isStoreOpen && !isDistrictManager)
            {
                answerPhone = false;
            }
        ...
        {
            double sumGiven = 0;
            double valueOfQuarters = numberQuarters * .25;
            double valueOfDimes = numberDimes * .1;
            double valueOfNickles = numberNickels * .05;
            double valueOfPennies = numberPennies * .01;

            sumGiven = numberDollars + valueOfQuarters + valueOfDimes + valueOfNickles + valueOfPennies;

            if (sumGiven >= itemPrice)
            {
                return true;
            }
        ...
        public string Hangman(string secretWord, char letterGuess)
        {
            ...
            {
                string guess = letterGuess.ToString(); // ex p
                string letterAtIndex = secretWord[i].ToString();


                if (guess.Equals(letterAtIndex))
                {
                    newString += guess;
        ...
        {
            int output;
            if (int.TryParse(userEntry, out output) && output >=1 && output <= maxValue)
            {
        ...
        public bool ValidateDouble(string userEntry)
        {
            ...
            if(double.TryParse(userEntry, out output))
            {
                return true;
            }
        ...
        public bool ValidateDoubleinRange(string userEntry, double minPrice, double maxPrice)
        {
            ...
            if (double.TryParse(userEntry, out output) && output >= minPrice && output <= maxPrice)
            {
                return true;
            }
        ...
        public bool ValidateItemEntry(string userEntry)
        {
            ...
            if (length > 3 || string.IsNullOrEmpty(userEntry) || length < 2)
            {
                return false;
            }

            char firstLetter = userEntry[0];
            bool isFirstIndexLetter = Char.IsLetter(firstLetter);
            ...
            if (!isFirstIndexLetter)
            {
             ...
             string newString = userEntry.Substring(1);
        ...
        public bool ValidateSelectionFromArray(string userEntry, string[] productSelections)
        {
            ...
            foreach(string s in productSelections)
            {
                if (userEntry.Equals(s))
                {
                    validSelection = true;
                }
            }
